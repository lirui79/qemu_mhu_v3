...
/* 8 字节对齐:setGICAddr 里 strd r0,r1,[r3] 要求目标地址 8 字节对齐。
 * 强制 8 对齐,防止 strd 写 GICD/GICR 基址失败。 */
struct GICv3_dist_if*       gic_dist __attribute__((aligned(8)));
struct GICv3_rdist_if*      gic_rdist __attribute__((aligned(8)));
...
uint32_t setIntType(uint32_t ID, uint32_t rd, uint32_t conf)
{
  uint32_t bank, tmp;
  (void)rd;
...
uint32_t setIntRoute(uint32_t ID, uint32_t mode, uint32_t affinity)
{
  ...
  #ifdef DEBUG
  printf("setIntRoute:: Routing INTID %d to mode=0x%x and affinity=0x%x\n", ID, mode, affinity);
  #endif
...
uint32_t clearIntActive(uint32_t ID, uint32_t rd)
{
  ...
  if (ID < 32)
  {
    ...
    gic_rdist[rd].sgis.GICR_ICACTIVER[0] = ID;
  ...
  else if (ID < 1020)
  {
    ...
    gic_dist->GICD_ICACTIVER[bank] = ID;
  }
  else
    return 1;
