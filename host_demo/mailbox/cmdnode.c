...
static cmdnode_t cnodes[32] = {0};


void          cmdnode_init(void) {
    int32_t i = 0;
    ...
    for (i = 0; i < 32; i++) {
        ...
        kref_set(&cnode->refcount, 0);//refcount 0
        rb_init_node(&cnode->node);
        cnode->cmdMsg    = NULL;
    }
}

static cmdnode_t* get_cmdnode(void) {
    int32_t i = 0;
    cmdnode_t* cnode = NULL;
    for (int i = 0; i < 32; i++) {
        cnode = &cnodes[i];
...
cmdnode_t*    cmdnode_alloc(uint32_t ackNum, uint32_t sessionID, uint64_t timeStamp, struct proc_obj *proc) {
    ...
    cmdnode_t* cnode = get_cmdnode();
    if (cnode == NULL) {
        ts_printf("ERROR:cmdnode get_cmdnode failed\n");
        return NULL;
    }
