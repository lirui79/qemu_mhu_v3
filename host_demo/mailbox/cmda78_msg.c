...
// 假设 configTICK_RATE_HZ 为 1000 (即 1ms 一个 tick)
    uint64_t time_ms = ticks * (1000 / configTICK_RATE_HZ);
    return time_ms;
}
...
static int32_t cmda78_send_wait_event(cmda78_session_t* session, struct proc_obj *proc, cmdMsg_t *cmdMsg, uint32_t timeout) {
	...
	retCode = wait_event_interruptible_timeout(cnode->wait, (cnode->code != CMD_ERR_UNKNOWN),	pdMS_TO_TICKS(timeout));
...
int32_t    cmda78_gen_close_session(struct proc_obj *proc, uint32_t coremask) {
    ...
    cmda78_session_t* cmd_session = (cmda78_session_t*)proc->session;
    ...
    cmdBody->procObj       = (uint64_t)proc;
    cmdBody->sessionID     = cmd_session->sessionID;
