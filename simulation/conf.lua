...
-- Execution file
EXEC_FILE_R52 = "../task_scheduler/build/bin/task_sched.bin"
EXEC_FILE_A76 = "../host_demo/build/bin/host_demo.bin"

-- R52 串口日志目录:优先用 run.sh 注入的 TS_LOG_DIR(原 ../log 不可写时
-- 会 fallback 到用户可写目录),默认仍为 <simulation>/../log
local ts_log_dir = os.getenv("TS_LOG_DIR") or (top() .. "../log")
...
-- A76 UART
local A76_UART0_BASE     = 0xD0000000
local A76_UART0_SIZE     = 0x00001000
...
platform = {
    ...
    charbackend_file_1 = {
        ...
        write_file = ts_log_dir .. "/ts_core0.log";
        read_file = "/dev/null";
    };

    charbackend_file_2 = {
        moduletype = "char_backend_file";
        write_file = ts_log_dir .. "/ts_core1.log";
        read_file = "/dev/null";
    };
    ...
    pl011_uart_0 = {
        ...
        backend_socket = {bind = "&charbackend_file_1.biflow_socket"},
    },

    -- Console UART1 (master_if_uart_1 @ 0x2FC61000), SPI 1 -> R52 GIC INTID 33
    pl011_uart_1 = {
        moduletype = "Pl011",
        dylib_path = "uart-pl011",
        target_socket = {address = UART1_BASE, size = UART1_SIZE, bind = "&router.initiator_socket"},
        irq = {bind = "&plugin_0.target_signal_socket_1"},
        backend_socket = {bind = "&charbackend_file_2.biflow_socket"},
    },
